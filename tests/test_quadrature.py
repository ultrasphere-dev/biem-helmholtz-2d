...
def test_kussmaul_martensen_kress_quadrature_exactness(
	...
	if f_case == "one":
		...
		f = xp.exp(1j * t)
		expected = -two_pi  # From docs/quadrature.typ: ∫ log(4 sin^2(t/2)) e^{i m t} dt = -2π/|m|.
	else:
		f = xp.ones_like(t) + xp.exp(1j * 3 * t) + xp.exp(-1j * 4 * t)
		expected = -(two_pi / 3 + two_pi / 4)  # Same identity applied to modes m=3 and m=-4.
...
def test_garrick_wittich_quadrature_exactness(xp: Any, device: Any, dtype: Any, f_case: str) -> None:
	...
	if f_case == "one":
		...
		f = xp.exp(1j * t)
		expected = -two_pi * abs(1)  # From docs/quadrature.typ: p.v.∫ cot(t/2) (e^{i m t})' dt = -2π|m|.
	else:
		f = xp.ones_like(t) + xp.exp(1j * 3 * t) + xp.exp(-1j * 4 * t)
		expected = -two_pi * (abs(3) + abs(-4))  # Sum of -2π|m| over modes m=3 and m=-4.
