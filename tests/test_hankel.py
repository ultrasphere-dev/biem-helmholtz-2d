...
from biem_helmholtz_2d._hankel import neumann_y1_y2, hankel_h1_h2
from biem_helmholtz_2d._quadrature import (
    log_cot_power_quadrature,
    trapezoidal_quadrature,
)
from biem_helmholtz_2d._scipy_wrapper import scipy_yv, scipy_hankel1
...
def test_split_quadrature_matches_trapezoidal(
    xp: Any, device: Any, dtype: Any, order: int, t_start_factor: float, split_type: str
) -> None:
    r"""
    Test if $\int_0^{2\pi} g(t) f(t)^{order} Y_{order}(f(t)) dt$ or
    $\int_0^{2\pi} g(t) f(t)^{order} H_{order}^{(1)}(f(t)) dt$ computed by
    the split quadrature matches the reference trapezoidal rule.
    """
    # approx
    n = 24
    ...
    f_ref = _f(t_ref)
    y_ref = (scipy_yv if split_type == "neumann" else scipy_hankel1)    (order, f_ref, device=device, dtype=dtype)
    integrand = _g(t_ref) * (f_ref**order) * y_ref
    expected = xp.sum(w_ref * integrand)

    # compare
    assert actual == pytest.approx(expected, rel=1e-4 if order == 0 else 1e-6)
